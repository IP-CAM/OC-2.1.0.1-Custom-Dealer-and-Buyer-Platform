...
class ControllerCustomerCustomer extends Controller {
	...
	public function getCustomers() {
		...
		$json = array();
			...
			$result = $this->model_customer_customer->getCustomersWithMainAddress();	
	...
	protected function getList() {
		...
		$data['storages'] = $this->model_storages_storages->getStorages();

		$data['loggedin_user_id'] = $this->user->getId();
...
// var_dump($filter_dealer);
		...
		// ARIEL CUSTOMER ROLE
		if (isset($this->request->get['filter_customer_role_id'])) {
			$filter_customer_role_id = $this->request->get['filter_customer_role_id'];
		} else {
			$filter_customer_role_id = null;
		}
		// ARIEL CUSTOMER ROLE END
		...
		$loggedin_user_id     = $this->user->getId();
		//GET LOGGED IN USER`S GROUP 
		$loggedin_user_group = $this->user->getGroupId();
		$data['loggedin_user_group'] = $loggedin_user_group;
		...
		$filter_data = array(
			...
			//ARILE CUSTOMER ROLE 
			'filter_customer_role_id' => $filter_customer_role_id,
			//ARIEL CUSTOMER ROLE END
			...
			//ARIEL IF DEALER IS LOGGED IN TO FILTER CUSTOMERS
			'loggedin_user_group'  		=> $loggedin_user_group,
		...
		foreach ($results as $result) {
			if (!$result['approved']) {$approve = $this->url->link('customer/customer/approve', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL');
			} else {
				$approve = '';
			}
		...
		$data['dealers'] = $this->model_user_user->getDealers();
		...
		// ARIEL CUSTOMER ROLE
		$data['sort_dealer'] = $this->url->link('customer/customer', 'token=' . $this->session->data['token'] . '&sort=dealer' . $url, 'SSL');
		// ARIEL CUSTOMER ROLE END
		...
		// ARIEL CUSTOMER ROLE
		$data['filter_customer_role_id'] = $filter_customer_role_id;
		//ARIEL CUSTOMER ROLE END
		...
		// ARIEL CUSTOMER ROLES
		$this->load->model('customer/customer_role');

		$data['customer_roles'] = $this->model_customer_customer_role->getCustomerRoles();
		
		// ARIEL CUSTOMER ROLES END
	...
	protected function getForm() {
		...
		if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			// ARIEL subemails
			$subemails = $this->model_customer_customer->getCustomerSubemails($this->request->get['customer_id']);
			// ARIEL END
		}
		...
		if (isset($this->request->post['customer_role_id'])) {
			$data['customer_role_id'] = $this->request->post['customer_role_id'];
		} elseif (!empty($customer_info)) {
			$data['customer_role_id'] = $customer_info['customer_role_id'];
		} else {
			$data['customer_role_id'] = 2;
		}
		
		// ARIEL CUSTOMER ROLES END

		// ARIEL SUBEMAILS
		if (isset($this->request->post['customer_subemails'])) {
			$data['customer_subemails'] = $this->request->post['customer_subemails'];			
		} elseif (!empty($customer_info)) {
			$data['customer_subemails'] = $subemails;
		} else {
			$data['customer_subemails'] = [];
		}
		if( $this->user->getGroupId() == 11 ){
			if (isset($this->request->post['dealer'])) {
				$data['dealer'] = $this->request->post['dealer'];
			} elseif (!empty($customer_info)) {
				$data['dealer'] = $customer_info['dealer_id'];
			} else {
				$data['dealer'] = $this->user->getId();
			}
		} else {
			if (isset($this->request->post['dealer'])) {
				$data['dealer'] = $this->request->post['dealer'];
	...
	public function login() {
		...
		if (isset($this->request->get['is_dealer'])) {
			$is_dealer = $this->request->get['is_dealer'];
		} else {
			$is_dealer = 0;
		}
		...
		if ($customer_info) {
			...
			if (isset($this->request->get['store_id'])) {
				...
				$store_id = 1;//Sofia alway or ???
			}

			// ARIEL STORAGES
			if (isset($this->request->get['storage'])) {
				$storage_id = $this->request->get['storage'];
			} else {
				$storage_id = 1;
			}
...
//ARIEL IS DEALER
			if ($store_info) {
				$this->response->redirect($store_info['url'] . 'index.php?route=account/login&token=' . $token . '&is_dealer=' . $is_dealer . '&storage='.$storage_id);
			} else {
				$this->response->redirect(HTTP_CATALOG . 'index.php?route=account/login&token=' . $token . '&is_dealer=' . $is_dealer . '&storage='.$storage_id);
			}
		} else {
