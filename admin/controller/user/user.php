...
class ControllerUserUser extends Controller {
	...
	protected function getForm() {
		...
		//ARIEL DEALER`S CUSTONERS
		//if edit user
		if (isset($this->request->get['user_id'])) {
		$data['user_customers'] = $this->model_user_user->getUserCustomers($this->request->get['user_id']);
		} elseif (!empty($user_info)) {
		// var_dump($this->request->post);
		// var_dump($this->request->get['user_id']);

			$data['user_customers'] = $this->model_user_user->getUserCustomers($user_info['user_id']);
		} else {
			$data['user_customers'] = [];
		}

		if (isset($this->request->post['user_new_customers'])) {
			$data['user_new_customers'] = $this->request->post['user_new_customers'];
		} else {
			$data['user_new_customers'] = [];
		}

		$this->load->model('customer/customer');
			
		$data['all_customers'] = $this->model_customer_customer->getCustomersWithMainAddress();
			
		$data['token'] = $this->session->data['token'];

		//get all customers in platform
		// $data['customers'] = $this->model_user_user_group->getUserGroups();
	...
	public function removeDealerCustomer() {
		$json = array();
		...
		if (isset($this->request->get['customer'])) {
			...
			$result = $this->model_user_user->removeDealerFromCustomer($this->request->get['customer']);

			
			
			if($result){
				$json['succes'] = 'Премахнахте клиент от дилъра!';
				$json['customer'] = $this->request->get['customer'];
			}
		}
