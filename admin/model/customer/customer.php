...
class ModelCustomerCustomer extends Model {
	//ARIEL IS DEALER
	public function getCustomersWithMainAddress()
	{
		$sql = "SELECT c.customer_id, c.email, CONCAT(c.firstname, ' ', c.lastname) AS name, a.company FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "address a ON (c.customer_id = a.customer_id) GROUP BY c.customer_id";
	...
	public function addCustomer($data) {
		// ARIEL IS DEALER
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW(), dealer_id='" . (int)$data['dealer'] . "'");
		...
		// ARIEL SUBEMAILS
			$this->db->query("DELETE FROM " . DB_PREFIX . "customer_subemails WHERE customer_id = '" . (int)$customer_id . "'");


			if(isset($data['customer_subemails']) && !empty($data['customer_subemails'])){
				foreach ($data['customer_subemails'] as $cse) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "customer_subemails SET customer_id = '" . (int)$customer_id . "', subemail = '" . $this->db->escape($cse['subemail']) . "'");
				}
			}
		//SUBEMAILS
	}
	...
	public function getCustomers($data = array()) {
		// var_dump($data);
		$sql = "SELECT c.customer_id, c.email, c.status, c.ip, c.date_added, c.approved, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group, cr.customer_role_type as customer_role, u.user_id,  CONCAT(u.firstname, ' ', u.lastname) AS dealer FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "customer_roles cr ON c.customer_role_id = cr.customer_role_id LEFT JOIN " . DB_PREFIX . "user u ON (u.user_id=c.dealer_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_dealer'])) {
			$implode[] = "c.dealer_id = '" . (int)$data['filter_dealer'] . "'";
		}

		//ARIEL FILTER BY DEALER
			if ($data['loggedin_user_group'] == 11){
				$implode[] = "(c.dealer_id) = '" . (int)($data['loggedin_user_id']) . "'";
			}
		//ARIEL END
...
// echo "</pre>";
	...
	public function getTotalCustomers($data = array()) {
		...
		// var_dump($data);
		if (!empty($data['filter_dealer'])) {
			$implode[] = "dealer_id = '" . (int)$data['filter_dealer'] . "'";
		}

		//ARIEL FILTER BY DEALER

			if (isset($data['loggedin_user_group']) && $data['loggedin_user_group'] == 11){
				$implode[] = "(dealer_id) = '" . (int)($data['loggedin_user_id']) . "'";
			}
		//ARIEL END
	...
	// ARIEL SUBEMAILS
	public function getCustomerSubemails($customer_id) {
		$sql = "SELECT subemail FROM " . DB_PREFIX . "customer_subemails WHERE customer_id=" . (int)$customer_id;
