...
class ModelUserUser extends Model {
	...
	public function getDealers(){
		...
		$sql = "SELECT user_id, CONCAT(firstname, ' ', lastname) AS name FROM `" . DB_PREFIX . "user` WHERE user_group_id=11";
...
// var_dump($query->rows);
	...
	public function getUserCustomers($user_id) {
		// var_dump($user_id);
		$query = $this->db->query("SELECT c.customer_id, c.firstname, c.lastname, c.email, a.company FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "address a ON c.customer_id = a.customer_id WHERE c.dealer_id = '" . (int)$user_id . "' GROUP BY c.customer_id");

		return $query->rows;
	}

	public function removeDealerFromCustomer($customer_id){
		$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET dealer_id = 0 WHERE customer_id = '" . (int)$customer_id . "'");
		return true;
	}
	//ARIEL END
	...
	public function editUser($user_id, $data) {
		...
		// var_dump($data['dealer_customer']);
		if( isset($data['user_new_customers']) ){
		foreach ($data['user_new_customers'] as $customer_id) {
			$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET dealer_id = '" . (int)($user_id) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
	}
	}
