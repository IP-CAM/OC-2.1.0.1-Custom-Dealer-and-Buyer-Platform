...
class ControllerCommonCart extends Controller {
	public function index() {
		...
		// var_dump($this->language->get('text_items'));
		...
		//ARIEL CHECK FOR SESSION STORAGE ID only IF IS DEALER
		if( isset($this->session->data['is_dealer']) ){
			...
			if( !empty($cart_products) ){
				...
				foreach ($cart_products as $cp) {
					if( $cp['storage_id'] != $storage_id ){
						$data['wrong_storage_entered'] = 'В количката ви има продукти, избрани от друг обект! Моля сменете обекта!';
						break;
					}
				}
			}
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			//ARIEL
			// array(0) { } array(1) { [0]=> array(3) { ["name"]=> string(14) "ПАРТИДА" ["value"]=> string(1) "2" ["type"]=> string(6) "select" } } 
