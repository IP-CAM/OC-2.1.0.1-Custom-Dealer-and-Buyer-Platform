		...
		//ARIEL CHECK FOR STORAGE AVAILABILITY IN EACH STORAGE
			...
			if( isset($this->session->data['is_dealer']) && isset($this->session->data['storage_id'])){
				...
				if( $this->session->data['storage_id'] == 1 ){
					...
					if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasSfStock() && !$this->config->get('config_stock_checkout'))) {
						$this->response->redirect($this->url->link('checkout/cart'));
					}
				} //CASE DOBRICH
				elseif( $this->session->data['storage_id'] == 2 ){
					
					if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasDobrStock() && !$this->config->get('config_stock_checkout'))) {
						$this->response->redirect($this->url->link('checkout/cart'));
					}
				}				
			}else{
