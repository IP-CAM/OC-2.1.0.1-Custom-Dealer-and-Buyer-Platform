<form class="form-horizontal">
  ...
  <?php } ?>
  <!-- ARIEL SUBEMAILS -->
   <?php if (isset($subemails)) { ?>
   <br>
  <div class="radio">
    <label>      
      Изберете E-MAIL за поръчка
    </label>
  </div>
  <br>
  <div id="payment-existing">
    <select name="subemail" class="form-control">
      <option value="<?= $default_email['email'] ?>" style="background-color: #1CBE27; color: #fff"><?= $default_email['email'] ?></option>
        <?php foreach ($subemails as $subemail) { ?>
       <option value="<?= $subemail['subemail'] ?>"><?= $subemail['subemail']?></option>
      <?php } ?>      
    </select>
  </div>
  <?php } ?>
