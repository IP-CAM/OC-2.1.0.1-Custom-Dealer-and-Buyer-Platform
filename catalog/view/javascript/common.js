	...
	'remove': function(key) {
		$.ajax({
			...
			success: function(json) {
				...
				let infoElement = $('span.price-edit-'+json.product_id);

				if( json.product_cart_data.length > 0 ){

					let product_data = json.product_cart_data, titleNew = 'Продуктът е добавен в количката! '; 
					...
					infoElement.attr('data-original-title', titleNew);
				}else{
					infoElement.attr('data-original-title', '');
