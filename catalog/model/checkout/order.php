...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		//ARIEL SUBTRACT QUANTITY FROM STORAGE ALSO
...
//ARIEL
			...
			// if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_product_query->rows as $order_product) {
					...
					$res = $this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
					// var_dump($res);
					// die;
					//ARIEL SUBTRACT FROM SOFIA STORAGE - BY TOTAL QUANTITY
					if( isset($storage_id) && $storage_id == 1 ){
						$this->sfdb->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
					}

					if( isset($storage_id) && $storage_id == 2 ){
						$this->dobrdb->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
					}

					$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product['order_product_id'] . "'");

					//INCORRECT - PRODUCT_OPTION_VALUE MAY BE DIFFERRENT
					...
					if( isset($storage_id) && $storage_id == 1){
							...
							// ARIEL SUBTRACT FROM SF STORAGE`- BY OPTIONS
							foreach ($order_option_query->rows as $option) {
								$this->sfdb->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
							}
					}	

					if( isset($storage_id) && $storage_id == 2){
						
							// ARIEL SUBTRACT FROM DOBRICH STORAGE - BY OPTIONS
							foreach ($order_option_query->rows as $option) {
								$this->dobrdb->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
							}
					}					
			...
			//ARIEL ACCORDING TO IS DEALER ORDER AND STORAGE 
			
			if( isset($storage_id) && $storage_id == 2 ){
				//IF STORAGE IS DOBRICH SET ORDER STATUS TO PROCESSING
				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '2', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");

				$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '2', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");

			} else {
				//ARIEL - IF NOT DEALER ORDER OR DEALER ORDER FOR SOFIA STORAGE - SET THE STATUS TO PENDING - CHANGE IT TO 1, AFTERWARDS!!!!
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '1', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
			}
